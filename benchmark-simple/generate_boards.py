...
CODENAMES_WORDS = [
    ...
    "LINE", "LINK", "LION", "LITTER", "LOCH NESS", "LOCK", "LOG", "LONDON", "LUCK", "MAIL",
    "MAMMOTH", "MAPLE", "MARBLE", "MARCH", "MASS", "MATCH", "MERCURY", "MEXICO", "MICROSCOPE", "MILLIONAIRE",
    "MINE", "MINT", "MISSILE", "MODEL", "MOLE", "MOON", "MOSCOW", "MOUNT", "MOUSE", "MOUTH",
    "MUG", "NAIL", "NEEDLE", "NET", "NEW YORK", "NIGHT", "NINJA", "NOTE", "NOVEL", "NURSE",
    "NUT", "OCTOPUS", "OIL", "OLIVE", "OLYMPUS", "OPERA", "ORANGE", "ORGAN", "PALM", "PAN",
    "PANTS", "PAPER", "PARACHUTE", "PARK", "PART", "PASS", "PASTE", "PENGUIN", "PHOENIX", "PIANO",
    "PIE", "PILOT", "PIN", "PIPE", "PIRATE", "PISTOL", "PIT", "PITCH", "PLANE", "PLASTIC",
    "PLATE", "PLATYPUS", "PLAY", "PLOT", "POINT", "POISON", "POLE", "POLICE", "POOL", "PORT",
...
def generate_boards(num_boards=5):
    boards = []
    for i in range(num_boards):
        # Pick 25 unique words randomly
        selected_words = random.sample(CODENAMES_WORDS, 25)
        
        # We need 9 red (targets) and 16 gray (bystanders)
        roles = ["red"] * 9 + ["gray"] * 16
        random.shuffle(roles)
        
        board_tiles = []
        for word, role in zip(selected_words, roles):
            board_tiles.append({
                "word": word,
                "color": role
            })
...
if __name__ == "__main__":
    ...
    output_file = "boards.json"
    with open(output_file, "w") as f:
        json.dump(boards, f, indent=2)
    print(f"Generated {len(boards)} boards and saved to {output_file}")
